...
class TransitGraph:
    ...
    def find_path_a_star(self, start, target, start_time, cost_criterion='t'):
        ...
        if target not in self.stops_coords:
            print(f"Target stop '{target}' not found in graph")
            return None, float('inf')
            
        # Check if start exists
        if start not in self.stops_coords:
            print(f"Start stop '{start}' not found in graph")
            return None, float('inf')
        ...
        if not self.is_potentially_reachable(start, target):
            print(f"Target '{target}' is not reachable from '{start}' according to precomputed data")
            # Even if precomputation says it's not reachable, let's still try (comment out return)
            # return None, float('inf')
        
        # Try to find direct connections first
        direct_result = self.find_direct_connections(start, target, start_time)
        if direct_result:
            print(f"Found direct connection from '{start}' to '{target}'")
        ...
        # Calculate spatial bounds for pruning - use more relaxed bounds
        if start in self.stops_coords and target in self.stops_coords:
            ...
            max_distance = haversine(start_lat, start_lon, target_lat, target_lon) * 4.0  # Allow 4x the direct distance (was 2.5x)
        else:
        ...
        nodes_explored = 0
        max_nodes = 50000  # Increased limit from 10000 to 50000
        
        print(f"Starting A* search from '{start}' to '{target}' at time {start_time}")
        ...
        while pq and nodes_explored < max_nodes:
            ...
            if nodes_explored % 1000 == 0:
                print(f"Explored {nodes_explored} nodes, queue size: {len(pq)}")
            ...
            if current == target:
                print(f"Found path to '{target}' after exploring {nodes_explored} nodes")
                return path, g
            
            # Dynamic bound adjustment - if we're exploring too widely, tighten the bound
            # Make this less aggressive
            if nodes_explored > 10000:  # Was 5000
                max_distance *= 0.9  # Was 0.8
            
            # Explore outgoing edges
            edges = sorted(self.graph.get(current, []), key=lambda e: e['dep_sec'])
            
            if not edges:
                print(f"No outgoing edges from stop '{current}'")
                
            for edge in edges:  # Sort by departure time
                if edge['dep_sec'] >= curr_time:
                    ...
                    # Make spatial pruning less aggressive
                    if next_stop in self.stops_coords and target in self.stops_coords:
                        # Only apply spatial pruning if we've already explored a lot of nodes
                        if nodes_explored > 5000 and self._is_too_far_from_path(current, next_stop, target, max_distance):
                            continue
                    ...
                    # Calculate heuristic with adaptive weight - make less aggressive
                    if cost_criterion == 't':
                        h = self.heuristic_time(next_stop, target) * 1.1  # Was 1.2
                    else:
        ...
        print(f"No path found after exploring {nodes_explored} nodes. Max limit: {max_nodes}")
        return None, float('inf')
    ...
    def _is_too_far_from_path(self, current, next_stop, target, max_distance):
        ...
        # Make this threshold less aggressive - was 1.5, now 2.5
        if next_to_target > current_to_target * 2.5 and next_to_target > max_distance:
            return True
