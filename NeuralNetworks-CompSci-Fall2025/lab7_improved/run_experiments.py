...
def run_experiments(output_dir='./results', subsample_fraction=1.0):
    """
    Run IMPROVED systematic experiments with bidirectional models, gradient clipping, etc.

    Experiments cover:
    - RNN type: BiLSTM (bidirectional LSTM)
    - Hidden dimension: [64, 128, 256]
    - Sequence truncation: [50, 100, 200] (optimal range from baseline)
    - Improvements: 100% data, bidirectional, dropout=0.7, grad_clip=1.0, weight_decay=1e-4

    Args:
        output_dir: Directory to save results
        subsample_fraction: Fraction of data to use (1.0 = 100% for improved version)
    """
    ...
    print("IMPROVED VERSION: BiLSTM, dropout=0.7, grad_clip=1.0, weight_decay=1e-4, scheduler")

    # Define parameter combinations (focused on what worked best)
    rnn_types = ['LSTM']  # Only LSTM since it was clearly better
    hidden_dims = [64, 128, 256]
    max_lengths = [50, 100, 200]  # Skip very short (20) and full length (both were bad)
    ...
    embedding_dim = 128
    batch_size = 64
    epochs = 30  # More epochs since we have more data
    learning_rate = 0.001
    dropout = 0.7  # Higher dropout for better generalization
    ...
    experiment_num = 0
    total_experiments = len(rnn_types) * len(hidden_dims) * len(max_lengths) * len(bidirectional)

    print(f"\nTotal experiments: {total_experiments}")
    print("="*80)

    for rnn_type, hidden_dim, max_len, use_bidir in product(rnn_types, hidden_dims, max_lengths, bidirectional):
        experiment_num += 1
        ...
        try:
            ...
            results, trained_model = train_model(
                ...
                learning_rate=learning_rate,
                weight_decay=weight_decay,
                patience=7,  # Higher patience for more data
                grad_clip=grad_clip,
                use_scheduler=True
            )

            # Save results
            bidir_str = 'bi' if use_bidir else ''
            exp_name = f"{bidir_str}{rnn_type.lower()}_h{hidden_dim}_len{max_len_str}"
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run IMPROVED RNN/LSTM experiments on IMDB')
    parser.add_argument('--quick', action='store_true', help='Run quick test')
    parser.add_argument('--output', type=str, default='./results', help='Output directory')
    parser.add_argument('--subsample', type=float, default=1.0,
                        help='Fraction of data to use (default: 1.0 = 100% for improved version)')
